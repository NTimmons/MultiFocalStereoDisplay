...
struct ContrEntry
{
	void InitDistanceInput( SceneModes _renderModeA, BlendModes _blendMode, StereoModes _stereo, EyeSeperationModes _eyeSep, float _da, float _db, float _dc, float _sa, float _sb, float _sc, int _answer, float _ra, float _rb)
	{
	...
	void InitInputComfort( SceneModes _renderModeA, StereoModes _stereoA, EyeSeperationModes _eyeSepA, BlendModes _blendModeA, ProjectionModes _projModeA, SceneModes _renderModeB, StereoModes _stereoB, EyeSeperationModes _eyeSepB, BlendModes _blendModeB, ProjectionModes _projModeB)
	{
		inputMode 	= eMode_INPUT_COMFORT;
...
class TestController
{
	...
	void PrintStats()
	{
		...
		for(unsigned int i = 0; i < m_WorkStack.size(); i++)
		{
			modeCount[m_WorkStack[i].inputMode]++;
		}

		for(unsigned int i = 0; i < count; i++)
		{
			std::cerr << names[i] << ": " << modeCount[i] << ".\n";
		}

		if(m_WorkStack.size())
			std::cerr << "Current Mode: " << names[m_active.inputMode] << ".\n";
		else
			std::cerr << "Empty Work Stack! \n";
	...
	void Update(float _time, int _input)
	{
		...
		else if( m_active.inputMode == eMode_INPUT || m_active.inputMode == eMode_INPUT_COMFORT)
		{
		...
		else if(m_active.inputMode == eMode_TIMER)
		{
			...
			{
				std::cerr << "Time remaining: " << m_active.timeMs - (_time - m_active.startTime) << ".\n";
			}
			return;
		}
	}
	...
	void OutputResult(bool _answer)
	{
		...
		{
			if(m_active.inputMode == eMode_INPUT)
				output.open("output_translation_perception.csv", std::ios::out | std::ios::app);
			else if(m_active.inputMode == eMode_INPUT_COMFORT)
				output.open("output_translation_comfort.csv", std::ios::out | std::ios::app);
		...
		else if( m_active.renderModeA == eScene_Rotation)
		{
			...
			if(m_active.inputMode == eMode_INPUT)
				output.open("output_rotation_perception.csv", std::ios::out | std::ios::app);
			else if(m_active.inputMode == eMode_INPUT_COMFORT)
				output.open("output_rotation_comfort.csv", std::ios::out | std::ios::app);
		...
		{
			if(m_active.inputMode == eMode_INPUT)
				output.open("output_static_perception.csv", std::ios::out | std::ios::app);
			else if(m_active.inputMode == eMode_INPUT_COMFORT)
				output.open("output_static_comfort.csv", std::ios::out | std::ios::app);
	...
	void Pop( float _time)
	{
		...
		if(m_WorkStack.size())
		{
			...
			std::cerr << "Remaining Entries: " << m_WorkStack.size() << "\n";
			PrintStats();
		}
		else
		{
